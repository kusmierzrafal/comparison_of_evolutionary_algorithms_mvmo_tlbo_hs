...
tlbo_stats_df.columns = ["min_result", "max_result", "mean_result"]
...
\\begin{{tabular}} {{| c | c | c | c |}} \hline
    rozmiar populacji & najlepszy wynik & najsłabszy wynik & średni wynik \\\\ \hline
    
"""

tlbo_latex_body_table = """
{pop_size} & {min_result} & {max_result} & {mean_result} \\\\ \hline
"""
...
for row in tlbo_stats_df.iterrows():
    tlbo_latex_table += tlbo_latex_body_table.format(pop_size=int(row[1]["pop_size"]), min_result=row[1]["min_result"], max_result=row[1]["max_result"], mean_result=row[1]["mean_result"])
...
mvmo_stats_df.columns = ["min_result", "max_result", "mean_result"]
...
\\begin{{tabular}} {{| c | c | c | c |}} \hline
    rozmiar archiwum & najlepszy wynik & najsłabszy wynik & średni wynik \\\\ \hline

"""

mvmo_latex_body_table = """
{n_best_size} & {min_result} & {max_result} & {mean_result} \\\\ \hline
"""
